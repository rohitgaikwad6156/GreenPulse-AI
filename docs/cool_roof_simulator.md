...
Separate measured roof albedos are not currently available. [`backend/app/simulation/config.py`](../backend/app/simulation/config.py) therefore declares **illustrative, calibration-required MVP assumptions** of `albedo_existing_roof = 0.20` and `albedo_cool = 0.65`, with provenance stating that no Pune measurement or selected product specification is supplied. These are not Pune measurements or guaranteed retrofit outcomes. The [U.S. Department of Energy cool-roof guide](https://www1.eere.energy.gov/buildings/publications/pdfs/corporate/coolroofguide.pdf) discusses a 0.65 initial reflectance threshold for some low-slope products; product choice, aging, dust, and local roof conditions still require measurement or documentation. Environment overrides remain calibration-required until evidence is documented.
...
The simulator returns baseline LST, scenario LST, `ΔLST = scenario − baseline`, `cooling = max(0, baseline − scenario)`, changed albedo, optional changed NDBI, retrofit area, training-support status, feasibility evidence, all changed features, and assumptions. Modified features beyond observed training minima/maxima are rejected; values outside p01–p99 but inside extrema are flagged as OOD. `eligible_roof_area_m2` may be omitted only when a verified per-cell capacity column and source hash are documented. A mapped roof footprint is not automatically called eligible. Positive ΔLST means the model predicts warming and cooling is zero. These are **model what-if estimates**, not observed or causal cooling.
...
Expected: six passing tests with **ARTIFICIAL TEST FIXTURES** only. They verify the area-weighted formula, unchanged non-roof features, optional NDBI, XGBoost re-prediction, combined joint prediction, feasibility and capacity-source checks, training-support/OOD behavior, and saved-model loading. Test predictions are not Pune climate results.
