...
4. **Final deployment fit:** after all outer scores are recorded, run a separate spatial Optuna search on the full dataset using the saved five block groups. Fit one final model on **all** retained rows with those final parameters and save it. The outer scores estimate the tuning workflow's geographic performance; they are **not** scores from predicting the rows on which this final model was fitted. Metadata records each feature's count, minimum, p01, p05, median, p95, p99, and maximum. Scenario code rejects modified vectors outside observed extrema and flags p01–p99 tails; this is a model-support guard, not causal evidence.
...
- `models/model_metadata.json`: feature list, UTC training time, SHA-256 dataset version, source date range, target and units, all five held-out scores and mean/std, final tuned and fixed model settings, reproducibility seed, each outer fold's selected hyperparameters, CRS, 30 m resolution, software versions, and nested spatial validation method. The baseline report must carry the same dataset checksum.
