...
Real `data/processed/greenpulse_ml_grid.parquet` and `data/processed/metadata.json` must exist from Step 12. Real `models/xgboost_lst.joblib` and `models/model_metadata.json` must exist from Step 15. The model metadata and dataset SHA-256, feature order, CRS, resolution, row count, target, and objective must match. A trusted model artifact is required because `joblib.load` executes pickle data. No real artifacts are supplied in this repository.
...
- `shap_global.json`: feature mean absolute SHAP (°C), grouped importance, all pairwise Pearson correlations, and pairs with absolute `r >= 0.85`.
- `shap_local.json`: `baseline_lST`, `predicted_lst`, `grid_id`, per-feature `raw_value`, signed `shap_value_c`, `direction`, grouped contributions, and UI waterfall/bar data.
...
For the local prediction, `predicted_lst ≈ baseline_lST + Σ shap_value_c`; code checks this to 0.001 °C absolute tolerance. Positive contributions have direction `warming`, negative contributions `cooling`. `warming_percentage_ui = 100 × positive_shap_value_c / Σ positive_shap_value_c`, with zero for cooling features. These percentages are display shares of positive model contributions, **not shares of physical heat causation**. Signed °C contributions are the primary result. Each category's local contribution is the signed sum of its member features. Global category importance is the sum of member mean absolute contributions, clearly labelled so it is not mistaken for a signed group effect.
...
Expected: three tests pass. They use **ARTIFICIAL TEST FIXTURES** in temporary directories. With this repository's current missing real model, running the explanation command should stop with `Real trained XGBoost model and model metadata are required; run Step 15 first`, and it must not create real explanation files.
