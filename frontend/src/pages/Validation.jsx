...
import { calculateValidation, getValidationDemo } from "../services/api.js";
  ...
  return <>
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,520px)_minmax(0,1fr)]">
      <section className="rounded-2xl border border-[#e4ebe3] bg-white p-5 shadow-[0_2px_14px_rgba(27,58,39,0.035)] sm:p-6">
        ...
        <p className="mt-2 text-xs leading-5 text-[#718276]">Use comparable March–May observations for the same 30 m cells. The API does not verify data provenance or causal assumptions.</p>
      ...
      <div className="space-y-5">
        {!report ? <section className="rounded-2xl border border-[#e4ebe3] bg-white p-8 text-center">
          ...
          <section className="rounded-2xl border border-[#e4ebe3] bg-white p-5 sm:p-6">
            <p className={`text-xs font-bold uppercase tracking-[0.1em] ${isDemo ? "text-[#94652e]" : "text-[#5c8465]"}`}>{report.label}</p>
            <h2 className="mt-2 text-lg font-semibold text-[#294532]">Predicted vs actual follow-up</h2>
            <p className="mt-2 text-xs leading-5 text-[#718276]">{report.source_note}</p>
          ...
          <section className="rounded-2xl border border-[#e4ebe3] bg-white p-5 sm:p-6">
            ...
            {report.performance ? <>
              <p className="mt-2 text-xs leading-5 text-[#718276]">{report.performance.residual_definition}; M = {report.performance.count_treated_cells} treated cells.</p>
